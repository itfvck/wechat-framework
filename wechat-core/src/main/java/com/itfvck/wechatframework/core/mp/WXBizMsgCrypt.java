 ...
 */
package com.itfvck.wechatframework.core.mp;
...
public class WXBizMsgCrypt {
     ...
     */
    public WXBizMsgCrypt(WechatConfig conf) {
        if (conf.getEncodingAESKey().length() != 43) {
            // throw new AesException(AesException.IllegalAesKey);
        } else {
            this.token = conf.getToken();
            this.appId = conf.getAppid();
            aesKey = Base64.decodeBase64(conf.getEncodingAESKey() + "=");
        }
    }
     ...
     *             执行失败，请查看该异常的错误码和具体的错误信息
     */
    public String encryptMsg(String replyMsg, WechatParam params) {
        // 加密
        String encrypt = null;
        String result = null;
        try {
            ...
            // 生成安全签名
            if (params.getTimestamp() == "") {
                params.setTimestamp(Long.toString(System.currentTimeMillis()));
            }
            String signature = SHA1.getSHA1(token, params.getTimestamp(), params.getNonce(), encrypt);
            result = XMLParse.generate(encrypt, signature, params.getTimestamp(), params.getNonce());
     ...
     *             执行失败，请查看该异常的错误码和具体的错误信息
     */
    public String decryptMsg(WechatParam params, String postData) throws AesException {
        ...
        // 验证安全签名
        String signature = SHA1.getSHA1(token, params.getTimestamp(), params.getNonce(), encrypt[1].toString());

        // 和URL中的签名比较是否相等
        // System.out.println("第三方收到URL中的签名：" + msg_sign);
        // System.out.println("第三方校验签名：" + signature);
        if (!signature.equals(params.getMsg_signature())) {
