...
import com.itfvck.wechatframework.api.coupon.manage.WxCardAPI;
import com.itfvck.wechatframework.api.coupon.manage.model.BatchGetCardRtnInfo;
import com.itfvck.wechatframework.api.coupon.manage.model.CardUpdate;
import com.itfvck.wechatframework.api.coupon.manage.model.CommCardRtnInfo;
import com.itfvck.wechatframework.api.coupon.manage.model.DelRtnInfo;
import com.itfvck.wechatframework.api.coupon.manage.model.GetCardDetailRtnInfo;
import com.itfvck.wechatframework.api.coupon.manage.model.GetCardRtnInfo;
import com.itfvck.wechatframework.api.coupon.qrcode.WxQrcodeAPI;
 ...
 * @version v1.0
 */
public class CardManageAPITest {
	...
	public static void main1(String[] args) {
		...
		for (String p : ls) {
			// 根据卡券ID，获取卡券详细
			GetCardDetailRtnInfo rtnInfo = WxCardAPI.doGetCardDetail(newAccessToken, p);
	...
	public static void main(String[] args) {
		String newAccessToken = WxAccessTokenAPI.getAccess_token(appid, appscret);
		try {
			GetticketRtn s = WxQrcodeAPI.doGetticket(newAccessToken);
	...
	public static List<String> getCardList(String newAccessToken) {
		...
		rtnInfo = WxCardAPI.doBatchGetCard(newAccessToken, 0, 50);
		if (rtnInfo.getErrcode() == 0) {
			// System.out.println("JwCardManageAPI.doBatchGetCard["+i+"]" +
			// ":成功");
			System.out.println("");
	...
	public static boolean doBatchGetCardTest(String newAccessToken) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				...
				rtnInfo = WxCardAPI.doBatchGetCard(newAccessToken, onecase.get("offset"), onecase.get("count"));
				if (rtnInfo.getErrcode() == 0) {
					pass++;
					// System.out.println("JwCardManageAPI.doBatchGetCard["+i+"]"
					// + ":成功");
	...
	public static boolean doDelCardTest(String newAccessToken) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				onecase = testCases.get(i);
				rtnInfo = WxCardAPI.doDelCard(newAccessToken, onecase.get("card_id"));
				if (rtnInfo.getErrcode() == 0) {
					pass++;
	...
	public static boolean doGetCardDetailTest(String newAccessToken) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				onecase = testCases.get(i);
				rtnInfo = WxCardAPI.doGetCardDetail(newAccessToken, onecase.get("card_id"));
				if (rtnInfo.getErrcode() == 0) {
					pass++;
	...
	public static boolean doGetCardTest(String newAccessToken) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				onecase = testCases.get(i);
				rtnInfo = WxCardAPI.doGetCard(newAccessToken, onecase.get("code"), onecase.get("card_id"));
				if (rtnInfo.getErrcode() == 0) {
					pass++;
	...
	public static boolean doModifystockCardTest(String newAccessToken) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				onecase = testCases.get(i);
				rtnInfo = WxCardAPI.doModifystockCard(newAccessToken, onecase.get("card_id"), Integer.parseInt(onecase.get("increase_stock_value")),
				        Integer.parseInt(onecase.get("reduce_stock_value")));
	...
	public static boolean doUnavailableCodeTest(String newAccessToken) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				onecase = testCases.get(i);
				rtnInfo = WxCardAPI.doUnavailableCode(newAccessToken, onecase.get("code"), onecase.get("card_id"));
				if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
					pass++;
	...
	public static boolean doUpdateCardTest(String newAccessToken) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				onecase = testCases.get(i);
				rtnInfo = WxCardAPI.doUpdateCard(newAccessToken, (CardUpdate) (onecase.get("cardUpdate")));
				if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
					pass++;
	...
	public static boolean doUpdateCodeTest(String newAccessToken) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				onecase = testCases.get(i);
				rtnInfo = WxCardAPI.doUpdateCode(newAccessToken, onecase.get("code").toString(), onecase.get("card_id").toString(), onecase.get("new_code").toString());
				if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
					pass++;
